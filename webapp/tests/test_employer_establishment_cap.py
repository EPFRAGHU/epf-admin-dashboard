...
def test_employer_establishment_cap_enforced_on_self_serve_creation(client, superadmin_session):
    ...
    res1 = employer.post("/api/establishments", json={"code": "CAPTEST01", "name": "Cap Test Est 1"})
    assert res1.status_code == 200, res1.text

    res2 = employer.post("/api/establishments", json={"code": "CAPTEST02", "name": "Cap Test Est 2"})
...
def test_employer_with_no_cap_is_unlimited(client, superadmin_session):
    ...
    for i in range(5):
        res = employer.post("/api/establishments", json={"code": f"UNLIM{i}", "name": f"Unlimited Est {i}"})
        assert res.status_code == 200, res.text

    res = employer.post("/api/establishments", json={"code": "UNLIM99", "name": "Over Cap"})
...
def test_raising_the_cap_unblocks_further_creation(client, superadmin_session):
    ...
    res1 = employer.post("/api/establishments", json={"code": "RAISECAP1", "name": "Raise Cap Est 1"})
    assert res1.status_code == 200, res1.text

    res_blocked = employer.post("/api/establishments", json={"code": "RAISECAP2", "name": "Raise Cap Est 2"})
    ...
    res2 = employer.post("/api/establishments", json={"code": "RAISECAP2", "name": "Raise Cap Est 2"})
...
def test_consultant_role_is_never_capped_even_if_max_establishments_field_sent(client, superadmin_session):
    ...
    for i in range(3):
        res = consultant.post("/api/establishments", json={"code": f"CONSCAP{i}", "name": f"Consultant Est {i}"})
        assert res.status_code == 200, res.text
...
def test_superadmin_creating_on_behalf_of_capped_employer_is_also_blocked(superadmin_session):
    ...
    res1 = superadmin_session.post("/api/establishments", json={
        "code": "SUPERCAP1", "name": "Super-created Est 1", "owner_user_id": employer_user["id"]
    })
    assert res1.status_code == 200, res1.text

    res2 = superadmin_session.post("/api/establishments", json={
